...
bool load(FILE* file, BYTE** content, size_t* length)
{
    ...
    int i = 0;
    while(true){
        if(fread(byte+i,1,1,file) == 1){
            i++;
            byte = realloc(byte,i+1);
...
const char* lookup(const char* path)
{
    ...
    if(strcasecmp(dot,".php") == 0)     return "text/x-php";
...
bool parse(const char* line, char* abs_path, char* query)
{
    ...
    for(;space_one[i+1] != '?' && space_one[i+1] != ' ';i++){
        if(space_one[i+1] == '"'){
            error(400);
            return false;
        }
        
        abs_path[i] = space_one[i+1];
    }
